...
DATASET_DIR = 'datasets/video_dataset'
...
# Проверка аргументов
if len(sys.argv) < 2:
    print("Использование: python create_dataset_from_video_yolo.py <путь_к_видео> [шаг_кадров]")
    print("  <путь_к_видео>  - путь к видеофайлу (обязательно)")
    print("  [шаг_кадров]     - сохранять каждый N-й кадр (опционально, по умолчанию 30)")
    sys.exit(1)

VIDEO_PATH = sys.argv[1]
FRAME_STEP = int(sys.argv[2]) if len(sys.argv) > 2 else 30
...
while True:
    ...
    if (key == ord('s') or detections) and detections:  # Принудительно или если есть объекты
        ...
        relative_img_path = os.path.join('datasets/video_dataset/images/train', base_name + '.jpg')
...
print(f"\nГотово. Извлечено {img_count} кадров из {VIDEO_PATH}")
